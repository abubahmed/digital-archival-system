...
import { z } from "zod";
import { saveToS3, existsInS3 } from "../../utils/s3Helpers.mjs";
import { markArchived, checkIfArchived } from "../../utils/dynamoHelpers.mjs";
...
export const fetchInstagramPosts = async ({
    ...
    items = items.filter(async (item) => {
        if (!item.type || !item.url || !item.timestamp || !item.id || (!item.images && !item.videoUrl && !item.displayUrl)) {
            ...
            return false;
        } else if (await checkIfArchived({ id: item.id, source: "instagram" })) {
            logger.debug(`Invalid item: already archived`);
            return false;
        }
        return true;
    });
...
export const runArchiverInstagram = async ({ after, limit }) => {
    ...
    try {
        ...
        for (const item of items) {
            ...
            if (await existsInS3({ key: mediaKey }) || await existsInS3({ key: metadataKey })) {
                logger.debug(`Item ${item.id} already exists in S3, skipping`);
                continue;
            }
            ...
            const markedArchived = await markArchived({
                id: item.id,
                source: "instagram",
            ...
            if (!markedArchived) {
                logger.error(`Failed to mark item ${item.id} as archived`);
                continue;
            }
...
runArchiverInstagram({ after: subtractDays(new Date(), 30), limit: 25 });